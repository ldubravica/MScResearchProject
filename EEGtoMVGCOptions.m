function [mvgc_open, mvgc_closed, ss_info_open, ss_info_closed] = EEGtoMVGCOptions(mvgc_format, load_mvgc_file, save_mvgc_file, plot_data, save_plots)
    ...
    % depressed_binary_labels = double(T.BDI >= 13);
    ...
    plot_mvgc(...
        ...
        sprintf('MVGC Differences (method: %s)', strrep(mvgc_format, '_', '-')));
    ...
    plot_mvgc(...
        ...
        {'Open ttest2', 'Open RLM (Sex+Age+Depression)', 'Open RLM (Sex+Age) + ttest2 (Residuals)', ...
         'Closed ttest2', 'Closed RLM (Sex+Age+Depression)', 'Closed RLM (Sex+Age) + ttest2 (Residuals)'}, ...
        sprintf('MVGC P-Values (method: %s)', strrep(mvgc_format, '_', '-')));
    ...
    plot_mvgc(...
        ...
        {'Open ttest2', 'Open RLM (Sex+Age+Depression)', 'Open RLM (Sex+Age) + ttest2 (Residuals)', ...
         'Closed ttest2', 'Closed RLM (Sex+Age+Depression)', 'Closed RLM (Sex+Age) + ttest2 (Residuals)'}, ...
        sprintf('MVGC T-Values (method: %s)', strrep(mvgc_format, '_', '-')));

    % Plot BHFDR-Corrected P-Values and T-Values

    plot_mvgc(...
        {mvgc_open_pvals_corrected, mvgc_closed_pvals_corrected, mvgc_open_tvals_masked, mvgc_closed_tvals_masked}, ...
        {'Open BHFDR P-Values', 'Closed BHFDR P-Values', 'Open Masked T-Values', 'Closed Masked T-Values'}, ...
        sprintf('MVGC BHFDR-Corrected P-Values (method: %s)', strrep(mvgc_format, '_', '-')));

    % plot_mvgc(...
    %     {mvgc_open_pvals_corrected, mvgc_closed_pvals_corrected}, ...
    %     {'Open Corrected P-Values', 'Closed Corrected P-Values'}, ...
    %     sprintf('MVGC P-Values BHFDR Corrected (method: %s)', strrep(mvgc_format, '_', '-')));
