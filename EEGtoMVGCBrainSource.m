function [mvgc_open, mvgc_closed, ss_info_open, ss_info_closed] = EEGtoMVGCBrainSource()
    ...
    % addpath(strrep(p, 'EEGtoMVGCBrainSource', 'EntRate/private/mvgc_v2.0'));
    % evalc('mvgc_startup;');
    ...
    function [pwcgc_values, info] = calculateMVGC(subj_data)
        ...
        [varmoaic, varmobic, varmohqc, varmolrt] = tsdata_to_varmo(subj_data, varmomax, 'LWR', [], []);
        ...
        [ssmo_svc, ssmo_max] = tsdata_to_sssvc(subj_data, 2*varmo, [], []);
        % [ssmo_svc, ssmo_max] = tsdata_to_ssmo(subj_data, 2*varmo);  % Incredibly slow
        ssmo = ssmo_svc;
        fprintf('Selected state-space model order (SVC): %d (max = %d)\n', ssmo, ssmo_max);
        
        % Estimate SS model
        fprintf('Estimating state-space model parameters...\n');
        [A, C, K, V] = tsdata_to_ss(subj_data, 2*varmo, ssmo);
    ...
    if LOAD_MVGC_FILE
        ...
        if exist('mvgc_values.mat', 'file')
            ...
            fprintf('No MVGC results found. Please run EEGtoMVGCBrainSource first.\n');
            return;
        end
