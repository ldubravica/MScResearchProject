function [mvgc_open, mvgc_closed, ss_info_open, ss_info_closed] = EEGtoMVGCRegion()
    ...
    % addpath(strrep(p, 'EEGtoMVGCRegion', 'EntRate/private/mvgc_v2.0'));
    % evalc('mvgc_startup;');
    ...
    if LOAD_MVGC_FILE
        ...
        if exist('mvgc_values_region.mat', 'file')
            load('mvgc_values_region.mat', 'mvgc_open', 'mvgc_closed');
            fprintf('MVGC results loaded successfully.\n');
        else
            fprintf('No MVGC results found. Please run EEGtoMVGCRegion first.\n');
            return;
        end
    ...
    % save(fullfile('mvgc_values_region.mat'), ...
